...
class Actions {
    constructor(config) {
        ...
        this.focusLeft = (cm, dm, window, column, grid) => {
            const leftColumn = grid.getLeftColumn(column) ?? grid.getLastColumn();
        ...
        this.focusRight = (cm, dm, window, column, grid) => {
            const rightColumn = grid.getRightColumn(column) ?? grid.getFirstColumn();
            if (rightColumn === null || rightColumn === column) {
                return;
            }
...
class Column {
    ...
    onWindowAdded(window, bottom) {
        ...
        if (this.width === 0) {
            const presetWidths = this.grid.config.presetWidths.getWidths(this.getMinWidth(), this.getMaxWidth());
            const defaultWidth = presetWidths[presetWidths.length - 1] ?? this.getMaxWidth();
            this.setWidth(defaultWidth, true);
        }
